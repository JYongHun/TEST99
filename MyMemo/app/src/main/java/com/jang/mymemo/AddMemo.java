package com.jang.mymemo;

import android.os.Bundle;
import android.view.View;
import android.widget.Button;
import android.widget.EditText;
import android.widget.TextView;
import android.widget.Toast;

import androidx.appcompat.app.AppCompatActivity;

import com.jang.mymemo.data.DatabaseHandler;
import com.jang.mymemo.model.Contact;

public class AddMemo extends AppCompatActivity {
    EditText name;
    EditText phone;
    Button btnAdd;

    String getName;
    String getPhone;
    int getId;
    int getPosition;

    TextView addTitle;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.add_memo);
        name = (EditText)findViewById(R.id.editText);
        phone = (EditText)findViewById(R.id.editContent);
        btnAdd = (Button)findViewById(R.id.btnSave);
        addTitle = (TextView)findViewById(R.id.addTitle);
        //카드뷰 클릭하면 화면 넘어갈 수 있도록, 멤버 변수 셋팅

        getName = getIntent().getStringExtra("name");
        getPhone = getIntent().getStringExtra("phone");
        getId = getIntent().getIntExtra("id",-1);

        if (getId == -1) {
            btnAdd.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    DatabaseHandler db = new DatabaseHandler(AddMemo.this);

                    if (name.getText().toString().trim().length() == 0 && phone.getText().toString().trim().length() == 0) {
                        Toast.makeText(AddMemo.this,"입력해야지 저장가능", Toast.LENGTH_SHORT).show();
                        return;
                    }

                    Contact new_contact = new Contact();
                    new_contact.setName(name.getText().toString().trim());
                    new_contact.setContent(phone.getText().toString().trim());

                    db.addContact(new_contact);

                    Toast.makeText(AddMemo.this,"잘 저장되었습니다.", Toast.LENGTH_SHORT).show();
                    finish();
                }
            });
        } else {
            name.setText(getName);
            phone.setText(getPhone);
            addTitle.setText("메모수정");

            btnAdd.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {

                    if (name.getText().toString().trim().length() == 0 && phone.getText().toString().trim().length() == 0) {
                        Toast.makeText(AddMemo.this,"입력해야지 저장가능", Toast.LENGTH_SHORT).show();
                        return;
                    }

                    DatabaseHandler db = new DatabaseHandler(AddMemo.this);

                    Contact update_contact = new Contact();
                    update_contact.setName(name.getText().toString().trim());
                    update_contact.setContent(phone.getText().toString().trim());
                    update_contact.setId(getId);

                    db.updateContact(update_contact);

                    Toast.makeText(AddMemo.this, "잘 수정되었습니다.", Toast.LENGTH_SHORT).show();
                    finish();
                }
            });
        }
    }
}
